...
import {
  Users, AlertCircle, TrendingDown, ArrowDownRight,
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          ...
          <p style={{ fontSize: '0.85rem', color: '#94a3b8', marginTop: '0.2rem' }}>
            {lang === 'ja' ? `${municipality.municipality_name} (${municipality.prefecture}) — 国立社会保障・人口問題研究所(NIPSSR)準拠コーホート要因法` : t.demo_subtitle}
          </p>
        </div>
      ...
      <div className="grid-cols-4">
        ...
        <div className="gov-card">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', marginTop: '0.4rem', fontSize: '0.75rem', color: '#f43f5e' }}>
            ...
            <span>ピーク比 {Math.round((municipality.population_current / municipality.population_peak) * 100)}% ({municipality.population_peak_year}年: {municipality.population_peak.toLocaleString()}人)</span>
          </div>
        </div>
        ...
        <div className="gov-card">
          ...
          <div style={{ fontSize: '0.75rem', color: '#94a3b8', marginTop: '0.4rem' }}>
            2056年予測: <strong style={{ color: '#f43f5e' }}>51.8%</strong> (2人に1人が高齢者)
          </div>
        </div>
        ...
        <div className="gov-card">
          ...
          <div style={{ fontSize: '0.75rem', color: '#94a3b8', marginTop: '0.4rem' }}>
            年間出生数: <strong style={{ color: '#f8fafc' }}>約28名</strong> / 小学校統廃合ライン
          </div>
        </div>
        ...
        <div className="gov-card">
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.5rem', marginTop: '0.5rem' }}>
            ...
            <span className="badge badge-red">要対策</span>
          </div>
          <div style={{ fontSize: '0.75rem', color: '#94a3b8', marginTop: '0.4rem' }}>
            財政力指数: <strong style={{ color: '#fcd34d' }}>{municipality.fiscal_independence_ratio}</strong> (自立困難)
          </div>
        </div>
      </div>
                  ...
                  content={({ payload, label }) => {
                    ...
                    return (
                      <div style={{ background: '#0e1424', border: '1px solid rgba(255,255,255,0.2)', padding: '0.5rem 0.75rem', borderRadius: '8px', fontSize: '0.75rem' }}>
                        <div style={{ fontWeight: 700, color: '#f8fafc', marginBottom: '0.25rem' }}>年齢階級: {label}</div>
                        <div style={{ color: '#38bdf8' }}>男性: {mVal.toLocaleString()} 人</div>
                        <div style={{ color: '#f43f5e' }}>女性: {fVal.toLocaleString()} 人</div>
                        <div style={{ color: '#a855f7', marginTop: '0.2rem', borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '0.2rem' }}>
                          合計: {(mVal + fVal).toLocaleString()} 人
                        </div>
                      </div>
                    );
                  }}
              ...
              <LineChart data={projections_summary} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                ...
                <Tooltip
                  content={({ payload, label }) => {
                    ...
                    return (
                      <div style={{ background: '#0e1424', border: '1px solid rgba(255,255,255,0.2)', padding: '0.5rem 0.75rem', borderRadius: '8px', fontSize: '0.75rem' }}>
                        <div style={{ fontWeight: 700, color: '#f8fafc', marginBottom: '0.25rem' }}>{label}年 推計値</div>
                        {payload.map((entry: any) => (
                          <div key={entry.name} style={{ color: entry.color }}>
                            {entry.name}: {Number(entry.value).toLocaleString()} 人
                          </div>
                        ))}
                      </div>
                    );
                ...
                />
                <Line type="monotone" dataKey="total" name="総人口" stroke="#f8fafc" strokeWidth={2.5} dot={false} />
                <Line type="monotone" dataKey="age_15_64" name="生産年齢 (15-64)" stroke="#6366f1" strokeWidth={2} dot={false} />
                <Line type="monotone" dataKey="age_65_plus" name="高齢人口 (65+)" stroke="#f59e0b" strokeWidth={2} dot={false} />
                <Line type="monotone" dataKey="age_75_plus" name="後期高齢 (75+)" stroke="#ef4444" strokeWidth={1.5} strokeDasharray="4 4" dot={false} />
                <Line type="monotone" dataKey="age_0_14" name="年少人口 (0-14)" stroke="#38bdf8" strokeWidth={2} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className="grid-cols-2">
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.78rem', textAlign: 'left' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.1)', color: '#94a3b8' }}>
                  <th style={{ padding: '0.5rem' }}>集落・町丁名</th>
                  <th style={{ padding: '0.5rem', textAlign: 'right' }}>人口</th>
                  <th style={{ padding: '0.5rem', textAlign: 'right' }}>高齢化率</th>
                  <th style={{ padding: '0.5rem', textAlign: 'right' }}>独居高齢</th>
                  <th style={{ padding: '0.5rem', textAlign: 'right' }}>5年増減</th>
                  <th style={{ padding: '0.5rem', textAlign: 'center' }}>都市計画区分</th>
                </tr>
              </thead>
              <tbody>
                {neighborhoods.map((n: any) => {
                  ...
                  const zoneText = n.compact_zone_status === 'maintain'
                    ? '維持・拠点'
                    : (n.compact_zone_status === 'consolidate' ? '集約誘導' : '段階的退役');
                  ...
                  return (
                    <tr key={n.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                      ...
                      <td style={{ padding: '0.5rem', textAlign: 'right', fontFamily: 'JetBrains Mono' }}>
                        {n.single_elderly_households} 世帯
                      </td>
        ...
        <div className="gov-card">
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.75rem' }}>
            ...
            <h2 style={{ fontSize: '1rem', fontWeight: 700, color: '#f8fafc' }}>
              自治体ベンチマーク比較 (Peer Comparison Mode)
            </h2>
          </div>
